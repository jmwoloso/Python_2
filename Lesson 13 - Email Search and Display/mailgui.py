...
"""
GUI for interacting with the email database.
"""
...
def get_date(s):
    """
    Assumes a date of form yyy-mm-dd, returns a corresponding datetime.date.
    """
    syear = s[:4]
    smonth = s[5:7]
    sday = s[8:]
...
class Application(Frame):
    ...
    def __init__(self, master=None):
        ...
        l1 = Label(self, 
                   text="Not Before (yyy-mm-dd):")
        l1.grid(row=1, 
                column=1, 
                sticky=N+S+E)
        
        self.mindate = Entry(self)
        self.mindate.grid(row=1, 
                          column=2, 
                          sticky=N+S+W)
        
        l2 = Label(self, 
                   text="Not After (yyy-mm-dd):")
        l2.grid(row=2, 
                column=1, 
                sticky=N+S+E)
        
        self.maxdate = Entry(self)
        self.maxdate.grid(row=2, 
                          column=2, 
                          sticky=N+S+W)
        
        l3 = Label(self,
                   text="Sender's E-mail Contains:")
        l3.grid(row=3,
                column=1,
                sticky=N+S+E)
        
        self.addsearch = Entry(self)
        self.addsearch.grid(row=3,
                            column=2,
                            sticky=N+S+W)
        
        l4 = Label(self, 
                   text="Sender's Name Contains:")
        l4.grid(row=4,
                column=1,
                sticky=N+S+E)
        
        self.namesearch = Entry(self)
        self.namesearch.grid(row=4,
                             column=2,
                             sticky=N+S+W)
        
        button = Button(self,
                        text="Search",
                        command=self.search_mail)
        button.grid(row=5,
                    column=2)
        
        self.msgsubs = Listbox(self,
                               height=10,
                               width=100)
        self.msgsubs.grid(row=8,
                          column=1,
                          columnspan=2)
        self.msgsubs.bind("<Double-Button-1>", 
                          self.display_mail)
        
        self.message = Text(self,
                            width=100)
        self.message.grid(row=9,
                          column=1,
                          columnspan=2)
    ...
    def search_mail(self):
        ...
        if not namesearch:
            namesearch = None
        self.msglist = msgs(mindate=mindate, 
                            maxdate=maxdate, 
                            addsearch=addsearch, 
                            namesearch=namesearch)
        self.msgsubs.delete(0, 
                           END)
        for pk, msg in self.msglist:
            self.msgsubs.insert(END, 
                                msg['subject'])
