#!/usr/bin/python3
#
# Demonstrating filedialog Module from Tkinter
#    dialogdemo.py
#
...
# Lesson 9 - Exercise 5
"""
Demonstrates functionality of the filedialog module in Tkinter
"""
from tkinter import *
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        d_button.config(width=12,
                        text="Directory Test",
                        command=self.askdir)
        d_button.pack(side=TOP)
        
        m_button = Button(self)
        m_button.config(width=12,
                        text="Messages Test",
                        command=self.messages)
        m_button.pack()
        
        c_button = Button(self)
        c_button.config(width=12,
                        text="Color Choice",
                        command=self.color)
        c_button.pack()
        
        l_button = Button(self)
        l_button.config(width=12,
                        text="Open File",
                        command=self.file_open)
        l_button.pack()
        
        s_button = Button(self)
        s_button.config(width=12,
                        text="Save File",
                        command=self.file_save)
        s_button.pack()
        
        self.QUIT = Button(self)
        self.QUIT.config(width=12,
                         text="Quit",
                         fg="red",
                         command=self.quit)
    ...
    def __init__(self, master=None):
        """Creating the parent frame for the app."""
